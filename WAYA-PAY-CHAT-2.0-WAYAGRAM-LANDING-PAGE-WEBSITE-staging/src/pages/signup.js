...
function SignUp() {
  ...
  return (
    <div>
      <TopNav />
      <div id='signIn' style={{ backgroundImage: 'url(loginBack.png)' }}>{response.show &&
        <div className={`alert alert-${response.status}`} role="alert">
          {response.message}
        </div>}

        <div className='showing'>
          <Rotate bottom left when={true}>
            <div id='signUp' className='row align-items-center justify-content-center'>
              ...
              <div className='col-sm-11 col-md-6'>
                <form onSubmit={handleSubmit}>
                  ...
                  <p style={{ color: '#2626BC', fontSize: '13px' }} className='py-1 col-sm-9 text-start pl-0'>Already have an account? <Link to='/signin'>Sign In</Link></p>
                  <input className="mr-2" type="checkbox" value="" id="t&c" />
                  <label className=" " htmlFor="t&c">
                    I am a Merchant
                  </label>
                  <div className='name  my-3 '>
                    <input name="firstName" type='text' className='' placeholder='First Name' onChange={handleChange} required />
                    <input name="surname" type='text' className='ml-2' placeholder='Surname' onChange={handleChange} required />
                  </div>
                  <div className=''>
                    ...
                    <div className='inputGroup text-left'>
                      <span className='iconeye' onClick={(e) => { togglePasswordVisibility(e, 'pass') }}> <img src='/see.png' /></span>
                      <input id='userpassword' name="password" type='password' placeholder='Password' onChange={handleChange} required />
                    </div>
                    <div className='inputGroup text-left'>
                      <span className='iconeye' onClick={(e) => { togglePasswordVisibility(e, 'confirm') }}> <img src='/see.png' /></span>
                      <input id='newuserpassword' name="confrim_password" type='password' placeholder='confirm Password' onChange={handleChange} required />
                    </div>
                    <div className='inputGroup text-left'>
                      <input name="phoneNumber" type='tel' placeholder='Phone Number' onChange={handleChange} required />
                    </div>
                    <div className='inputGroup text-left'>
                      <input name="referenceCode" type='text' placeholder='Reference code' onChange={handleChange} />
                    </div>
                    ...
                    <div className=' mt-3'>
                      <button type='submit' id='signupBtn' className='orange '>{loading ? "Loading..." : "Sign Up"} <span><img src='/rightArr.png' /></span></button>
                    </div>
